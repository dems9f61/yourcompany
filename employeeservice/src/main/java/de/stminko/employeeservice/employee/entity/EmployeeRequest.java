...
/**
 * Represents a request object for employee-related operations.
 * This record encapsulates various fields like email address, first name, last name,
 * birthday, and department name for handling employee data processing.
 * <p>
 * It is annotated with {@code JsonView} and validation constraints to facilitate serialization
 * and validation in different contexts like POST, PUT, PATCH, and GET requests.
 * This record is particularly designed to be used with Spring Web MVC and Jackson for
 * RESTful web services.
 * <p>
 * Usage of {@code JsonIgnoreProperties} ensures that any unknown properties in the JSON request
 * do not cause deserialization issues.
 * <p>
 * Fields:
 * - emailAddress: The employee's email address. Must match the defined EMAIL_REGEX pattern.
 * - firstName: The employee's first name. Required for PUT requests.
 * - lastName: The employee's last name. Required for PUT requests.
 * - birthday: The employee's date of birth in ZonedDateTime format. Required for PUT requests.
 * - departmentName: The name of the department the employee belongs to. Required for POST and PUT requests.
 * <p>
 * Note:
 * The EMAIL_REGEX pattern defines the acceptable format for the email address field:
 * - this regex ensures that the provided email has a username part with allowed characters,
 * followed by an @ symbol, then a domain part with allowed characters, a period, and a top-level domain of at least two letters.
 *
