 ...
 * creating, updating, finding, and deleting employee records. It interacts with {@link EmployeeRepository}
 * for data persistence and {@link DepartmentService} for department-related operations. Additionally,
...
public class EmployeeService {
     ...
     * <p>
     * Validates the provided {@link EmployeeRequest}, checks for the uniqueness of the email address,
     * and persists the new employee record. After successful creation, an event is published
     * via {@link EmployeeEventPublisher}.
     * </p>
     *
     ...
     * <p>
     * Searches for an employee using the provided ID. If the employee is not found,
     ...
     * <p>
     * Updates the employee identified by the provided ID with the new data from the request object.
     * Validates the request and applies the changes. An event is published if any change occurs.
     * </p>
     *
     * @param id            the unique identifier of the employee
     ...
     * <p>
     * Similar to {@link #doFullUpdate}, but only updates the fields provided in the update request.
     * An event is published if any change occurs.
     * </p>
     *
     * @param id            the unique identifier of the employee
     ...
     * <p>
     * This method removes an employee record from the system using the provided ID. It first verifies
     * that an employee with the given ID exists, throwing a {@link NotFoundException} if no such employee
     * is found. After successful deletion, an event is published via {@link EmployeeEventPublisher} to
     * indicate that an employee has been deleted.
     * </p>
     *
    ...
    private boolean hasDepartmentChangedAfterUpdate(EmployeeRequest updateParameter, Employee employee) {
        ...
        if (StringUtils.isNotBlank(newDepartmentName) && !StringUtils.equals(newDepartmentName, employee.getDepartment().getDepartmentName())) {
            Department department = this.departmentService.findByDepartmentNameOrElseThrow(newDepartmentName, BadRequestException.class);
