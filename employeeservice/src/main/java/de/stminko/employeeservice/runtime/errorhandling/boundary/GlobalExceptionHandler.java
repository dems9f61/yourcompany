...
/**
 * Global exception handler for the Spring application.
 * This class is annotated with @ControllerAdvice to provide centralized exception handling across all @RequestMapping methods.
 * It captures and handles various types of exceptions, converting them into a structured format for client responses.
 * <p>
 * Features:
 * - Logs the details of handled exceptions, including HTTP request information.
 * - Resolves appropriate HTTP status codes based on the exception type.
 * - Serializes error information into a consistent response format.
 * - Handles constraint violations by converting them into {@link ConstraintViolationInfo} objects.
 * <p>
 * Methods:
 * - handleException: Captures and processes generic exceptions, returning a structured error response.
 * - createErrorResponseBody: Constructs the error response body based on the exception and request details.
 * - resolveHttpResponseStatus: Determines the appropriate HTTP status code based on the exception type.
 * - getResponseContentType: Resolves the response content type based on the request's Accept header.
 * - getConstraintViolationInfos: Extracts constraint violation details from exceptions.
 * <p>
 * Usage:
 * This class is automatically used by Spring to handle exceptions thrown by controllers.
 * It simplifies error handling by providing a consistent structure for error responses.
 *
